...
// ===========================> Functions Prototype <===============================
void conv1d(const int   *data, const int   *filter, int   *map_out, int filter_size,
...
void conv1d(const int   *data, const int   *filter, int   *map_out, const int filter_size,
...
//    int   *layers_out[8] = {0};
    ...
    filter = enc_w[0];
    conv1d(layer_in, filter, first_conv, 7, map_size[0], depth_size[0], depth_size[1], 2);
    ...
    for (int block = 0; block < 4; block++) {
        int* conv1d_1 = (int   *) malloc(map_size[block*2 + 2] * depth_size[block* 2 + 2] * sizeof(int  ));
        filter = enc_w[block*2 + 1];
        conv1d(layer_in, filter, conv1d_1, 3, map_size[block*2 + 1], depth_size[block*2 + 1],
               depth_size[block*2 + 2], 2);
        int* conv1d_2 = (int   *) malloc(map_size[block*2 + 3] * depth_size[block* 2 + 3] * sizeof(int  ));
        filter = enc_w[block*2 + 2];
        conv1d(conv1d_1, filter, conv1d_2, 3,  map_size[block*2 + 2], depth_size[block*2 + 2],
               depth_size[block*2 + 3], 1);

        int* conv1d_skip = (int   *) malloc(map_size[block*2 + 3] * depth_size[block* 2 + 3] * sizeof(int  ));
        filter = dec_w[block];
        conv1d(layer_in, filter, conv1d_skip, 7, map_size[block*2 + 1], depth_size[block*2 + 1],
               depth_size[block*2 + 3], 2);

        skip_add(conv1d_2, conv1d_skip, map_size[block*2 + 3] * depth_size[block* 2 + 3]);

        layer_in = conv1d_skip;
    }
    int* fully_connected = malloc(2 * sizeof(int  ));
    conv1d(layer_in, fc_w, fully_connected, 7, map_size[9], depth_size[9], 2, 1);
